...
@Component({
    selector: 'source-value-detail',
    templateUrl: './source-value-detail.component.html',
    styleUrls: ['./source-value-detail.component.scss']
})
...
export class SourceDetailComponent  {
                        ...
                        return this.sourceService.getSourseDocument()
                            ...
                            .then((data: any) => {
                                ...
                                if(!this.currencyOneStatic){
                                    ...
                                    this.currencyOneStatic = this.viewMode !== 'add' ? data.VALUES[Number(this.viewName)]
                                        : {
                                            DESCRIPTIONS:[],
                                            VALUE: ''
                                        };
    ...
    saveCondition(){
        ...
        if(!data.VALUE || data.DESCRIPTIONS.length < 1) {
            !data.VALUE && MessageToast.showError("Source code is required");
        ...
        if(this.viewMode == 'add') {
            ...
            this.currencyStatic.VALUES.push(data);
            this.sourceService.putSourseValues(this.currencyStatic)
                .then(() => this.router.navigateByUrl('configuration/field-configuration/source-values'))
                .catch((err:any) => MessageToast.showError(err.error ? err.error.MESSAGE : 'Error add'))
        } else {
            this.currencyStatic.VALUES.splice(Number(this.viewName), 1, data)
            this.sourceService.putSourseValues(this.currencyStatic)
                .then(() => this.router.navigateByUrl('configuration/field-configuration/source-values'))
