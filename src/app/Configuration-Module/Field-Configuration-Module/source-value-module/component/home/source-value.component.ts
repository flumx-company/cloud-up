...
export class SourceComponent implements OnInit {
        ...
        this.dataSource.value = new DataSource({
            store: new CustomStore({
                load: (loadOptions: any) => {
                    return this.sourceService.getSourseDocument()
                        .toPromise()
                        .then((data: any) => {
                ...
                remove: (obj: any) => {
                    ...
                    statics.VALUES.find((item:any, index: any) => {
                        const find = item.id == obj.id;
                        find && (mainIndex = `${index}`);
                        return find;
                    });
                    mainIndex && statics.VALUES.splice(Number(mainIndex), 1);
                    statics.VALUES = statics.VALUES.map((item:any) => {
                    ...
                    return this.sourceService.putSourseValues(statics)
                        .then(() => this.dataGrid.instance.refresh())
                        .catch(() => {
                            MessageToast.showError('Source removed successfully');
                            return {data: [], totalCount: 0};
                        });
                }
            })
        ...
        const arrayButton = [
            {
                ...
                options: {
                    hint: 'Add',
                    icon: "add", onClick: () => this.router.navigateByUrl('configuration/field-configuration/source-values/add')
                },
                location: "after"
            },
            {
    ...
    click_in_Icon(e: any, name: string) {
        name === 'edit'   && this.router.navigateByUrl(`configuration/field-configuration/source-values/edit/${e.rowIndex}`)
        name === 'delete' && this.dataGrid.instance.deleteRow(e.rowIndex);
        name === 'add'    && this.router.navigateByUrl('configuration/field-configuration/source-values/add')
    }
    ...
    newDoc(){
        ...
        this.sourceService.setNewSourseDocument(data)
